...
document.querySelector("#opener3").addEventListener('load',function (e) {
...
function syncCheckbox(node, code){
  ...
  xhr.open("get", `/users/syncCheckbox?code=${code}`, false);
  xhr.send(null);
  result = xhr.responseText


  node.forEach(function (name){
    if (result.indexOf(name.getAttribute("code")) != -1)
...
console.log(node.querySelector("input").checked)
	if (node.querySelector("input").checked)
		xhr.open("get", `/users/insert?code=${node.getAttribute("code")}&dong=${node.querySelector("label").querySelector(".label").innerHTML}`, false);
	else
		xhr.open("get", `/users/delete?code=${node.getAttribute("code")}&dong=${node.querySelector("label").querySelector(".label").innerHTML}`, false);
//		
//
//
  xhr.send(null);
