...
public class apartmentDAOImpl implements apartmentDAO {
	...
	@Override
	public ArrayList<apartmentDTO> getUserFavorApartments(String id) throws SQLException, Exception {
		// TODO Auto-generated method stub
		ArrayList<apartmentDTO> list = new ArrayList<apartmentDTO>();
		
		Connection con = DBUtil.getConnection();
		PreparedStatement pst = con.prepareStatement("SELECT apt, floor, area, dong, transactionAmount, jibun FROM transaction " + 
				"where sigunguCode in (select LEFT(code,5) from userFavor where id = ?)" + 
				"and dong in (select LEFT(dong,5) from userFavor where id = ?)");
		pst.setString(1, id);
		pst.setString(2, id);System.out.println(pst.toString());
...
//					+ "`serialNumber` VARCHAR(100) NOT NULL PRIMARY KEY,"
		...
		pst.setString(1, "userFavor");System.out.println(pst.toString());
	...
	public int deleteFavApt(String id, String code, String dong) throws SQLException, Exception {
		Connection con = DBUtil.getConnection();
		String q = "Delete from userfavor where id=? and code = ? and dong = ?";
	...
	public ArrayList<String> getUserFavorSync(String id, String code) throws SQLException, Exception {
		...
		pst.setString(1, id); System.out.println(pst.toString());
